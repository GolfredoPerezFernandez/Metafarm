...
const Mint = () => {
  ...
  const [mintBatchStartId, setMintBatchStartId] = React.useState<any>(null);
  const [mintBatchEndId, setMintBatchEndId] = React.useState<any>(null);
  const [marketItems, setMarketItems] = React.useState<any>(null);
  ...
  const onCreateMarketItemBatch = async() => {
    ...
    await contractProcessor.fetch({
        ...
        onError: (err:any) => { alert("FAIL check console"); console.log(err) }
    })
}

const numbersToArray = (begin:any, end:any) => {
  let result = [];
...
const resetMintBatch = () => {
  ...
  return (
    <VStack paddingBottom={20} alignItems={'center'} justifyContent={'flex-start'} w={'full'} h={'full'}>
     
        <Heading size="md" marginBottom={3}>
      STEP 1: Mint NFT Collection [a,a+1,...b]
      </Heading>
      <Text>
      from id a to b
      </Text>  
      <List justifyContent={'center'} alignItems='center'  spacing={3}>
      <VStack justifyContent={'center'} alignItems='center' >
         <ListItem>   <>
      <Text mb='8px'>Value: {mintBatchStartId}</Text>
      <Input
        value={mintBatchStartId}
      ...
      />
        ...
        <ListItem>   <>
      <Text mb='8px'>Value: {mintBatchEndId}</Text>
      <Input
        value={mintBatchEndId}
    ...
    <Button style={{alignSelf:'center'}} size="lg"  onClick={onMintBatch} colorScheme="yellow">
     Mint NFTs
    </Button> 
   </VStack>
      </List>
<Stack style={{height:50}}></Stack>
      <Heading size="md" marginTop={3} marginBottom={3}>
    STEP 2: Add to Market
    </Heading>  
      <Text>
      {`{"tokenIds": [], "prices": []}`}
      </Text>  
      <List   spacing={3}>    
        ...
        <ListItem>   <>
      <Text mb='8px'>Value: {marketItems}</Text>
      <Input
    ...
    <Button style={{alignSelf:'center',marginLeft:4}} size='lg' onClick={onCreateMarketItemBatch} colorScheme="yellow">
     Add to Market
    </Button> 
    </VStack>
      </List>
